...
export async function PUT(
  ...
  if (!clave) {
    return NextResponse.json({ message: "El campo 'clave' es obligatorio" }, { status: 400 });
  }
  ...
  try {
    const backendRes = await fetch(
      `${backendBase}/api/auth/usuarios/${encodeURIComponent(usuario)}/cambiar-clave`,
      {
        method: "PUT",
        headers: {
    ...
    return NextResponse.json(
      (data as object) ?? { message: "Contraseña actualizada" },
      { status: 200 }
    );
  } catch (err: unknown) {
